...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FourOhFourCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "FourOhFourCoin Signed Message:\n";
...
{
    // FourOhFourCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // FourOhFourCoin: 420k blocks in ~1 year

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // FourOhFourCoin: every 2 min
static const int64 nTargetSpacing = 2.5 * 60; // FourOhFourCoin: 40 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // FourOhFourCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // FourOhFourCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Herbert-inspired Altcoin FOF makes its debut to rival controversial MaxCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // FourOhFourCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FourOhFourCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FourOhFourCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FourOhFourCoinMiner(CWallet *pwallet)
{
    printf("FourOhFourCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running FourOhFourCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FourOhFourCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FourOhFourCoinMiner, pwallet));
}
